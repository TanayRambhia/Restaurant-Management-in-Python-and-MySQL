...
def send():
    if textReceipt.get(1.0,END)=='\n':
        ...
        root2.config(bg='cornflowerblue')
        root2.geometry('485x620+50+50')

        logoImage=PhotoImage(file='sender.png')
        label=Label(root2,image=logoImage,bg='cornflowerblue')
        label.pack(pady=5)

        numberLabel=Label(root2,text='Mobile Number',font=('arial',18,'bold underline'),bg='cornflowerblue',fg='white')
        numberLabel.pack(pady=5)

        numberfield=Entry(root2,font=('helvetica',22,'bold'),bd=3,width=24)
        numberfield.pack(pady=5)

        billLabel = Label(root2, text='Bill Details', font=('arial', 18, 'bold underline'), bg='cornflowerblue', fg='white')
        ...
        sendButton=Button(root2,text='SEND',font=('arial',19,'bold'),bg='white',fg='cornflowerblue',bd=7,relief=GROOVE
                          ,command=send_msg)
...
def receipt():
    ...
    if costoffoodvar.get() != '' or costofcakesvar.get() != '' or costofdrinksvar.get() != '':
        ...
        if e_fish.get()!='0':
            textReceipt.insert(END,f'Margeritta Pizza\t\t\t{int(e_fish.get())*100}\n\n')

        if e_chawal.get() != '0':
            textReceipt.insert(END, f'Vada Pav:\t\t\t{int(e_chawal.get()) * 30}\n\n')

        if e_sabji.get() != '0':
            textReceipt.insert(END, f'Sabji:\t\t\t{int(e_sabji.get()) * 50}\n\n')

        if e_paneer.get() != '0':
            textReceipt.insert(END, f'Paneer Chilli:\t\t\t{int(e_paneer.get()) * 100}\n\n')

        if e_kebab.get() != '0':
            textReceipt.insert(END, f'Burger:\t\t\t{int(e_kebab.get()) * 40}\n\n')

        if e_chicken.get() != '0':
            textReceipt.insert(END, f'Paneer Tikka Roll:\t\t\t{int(e_chicken.get()) * 120}\n\n')

        if e_mutton.get() != '0':
            textReceipt.insert(END, f'Schezwan Fried Rice:\t\t\t{int(e_mutton.get()) * 120}\n\n')
...
root=Tk()
root.geometry('1270x690+50+50')
root.resizable(1,1)
root.title('Restaurant Management System ')

root.config(bg='aqua')

topFrame=Frame(root,bd=10,relief=RIDGE,bg='cyan')
topFrame.pack(side=TOP)

labelTitle=Label(topFrame,text='Restaurant Management System',font=('arial',30,'bold'),fg='yellow',bd=9,
                 bg='darkcyan',width=51)
labelTitle.grid(row=0,column=0)

#frames

menuFrame=Frame(root,bd=10,relief=RIDGE,bg='lightsteelblue')
menuFrame.pack(side=LEFT)

costFrame=Frame(menuFrame,bd=4,relief=RIDGE,bg='paleturquoise',pady=10)
costFrame.pack(side=BOTTOM)

foodFrame=LabelFrame(menuFrame,text='Food',font=('arial',19,'bold'),bd=10,relief=RIDGE,fg='darkslategray',)
foodFrame.pack(side=LEFT)

drinksFrame=LabelFrame(menuFrame,text='Drinks',font=('arial',19,'bold'),bd=10,relief=RIDGE,fg='darkslategray')
drinksFrame.pack(side=LEFT)

cakesFrame=LabelFrame(menuFrame,text='Cakes',font=('arial',19,'bold'),bd=10,relief=RIDGE,fg='darkslategray')
cakesFrame.pack(side=LEFT)

rightFrame=Frame(root,bd=15,relief=RIDGE,bg='cadetblue')
rightFrame.pack(side=RIGHT)

calculatorFrame=Frame(rightFrame,bd=1,relief=RIDGE,bg='cadetblue')
calculatorFrame.pack()

recieptFrame=Frame(rightFrame,bd=4,relief=RIDGE,bg='cadetblue')
recieptFrame.pack()

buttonFrame=Frame(rightFrame,bd=3,relief=RIDGE,bg='cadetblue')
buttonFrame.pack()
...
fish=Checkbutton(foodFrame,text='Margeritta Pizza',font=('arial',18,'bold'),onvalue=1,offvalue=0,variable=var3
                 ,command=fish)
...
kebab=Checkbutton(foodFrame,text='Burger',font=('arial',18,'bold'),onvalue=1,offvalue=0,variable=var5
                  ,command=kebab)
kebab.grid(row=4,column=0,sticky=W)

chawal=Checkbutton(foodFrame,text='Vada Pav',font=('arial',18,'bold'),onvalue=1,offvalue=0,variable=var6
                   ,command=chawal)
chawal.grid(row=5,column=0,sticky=W)

mutton=Checkbutton(foodFrame,text='Schezwan Fried Rice',font=('arial',18,'bold'),onvalue=1,offvalue=0,variable=var7,
                   command=mutton)
mutton.grid(row=6,column=0,sticky=W)

paneer=Checkbutton(foodFrame,text='Paneer Chilli',font=('arial',18,'bold'),onvalue=1,offvalue=0,variable=var8
                   ,command=paneer)
paneer.grid(row=7,column=0,sticky=W)

chicken=Checkbutton(foodFrame,text='Paneer Tikka Roll  ',font=('arial',18,'bold'),onvalue=1,offvalue=0,variable=var9
                    ,command=chicken)
...
labelCostofFood=Label(costFrame,text='Cost of Food',font=('arial',16,'bold'),bg='teal',fg='white')
...
labelCostofDrinks=Label(costFrame,text='Cost of Drinks',font=('arial',16,'bold'),bg='teal',fg='white')
...
labelCostofCakes=Label(costFrame,text='Cost of Cakes',font=('arial',16,'bold'),bg='teal',fg='white')
...
labelSubTotal=Label(costFrame,text='Sub Total',font=('arial',16,'bold'),bg='teal',fg='white')
labelSubTotal.grid(row=0,column=2)

textSubTotal=Entry(costFrame,font=('arial',16,'bold'),bd=6,width=14,state='readonly',textvariable=subtotalvar)
textSubTotal.grid(row=0,column=3,padx=41)

labelServiceTax=Label(costFrame,text='Service Tax',font=('arial',16,'bold'),bg='teal',fg='white')
...
labelTotalCost=Label(costFrame,text='Total Cost',font=('arial',16,'bold'),bg='teal',fg='white')
...
buttonTotal=Button(buttonFrame,text='Total',font=('arial',14,'bold'),fg='white',bg='teal',bd=3,padx=5,
                   command=totalcost)
buttonTotal.grid(row=0,column=0)

buttonReceipt=Button(buttonFrame,text='Receipt',font=('arial',14,'bold'),fg='white',bg='teal',bd=3,padx=5
                     ,command=receipt)
buttonReceipt.grid(row=0,column=1)

buttonSave=Button(buttonFrame,text='Save',font=('arial',14,'bold'),fg='white',bg='teal',bd=3,padx=5
                  ,command=save)
buttonSave.grid(row=0,column=2)

buttonSend=Button(buttonFrame,text='Send',font=('arial',14,'bold'),fg='white',bg='teal',bd=3,padx=5,command=send)
buttonSend.grid(row=0,column=3)

buttonReset=Button(buttonFrame,text='Reset',font=('arial',14,'bold'),fg='white',bg='teal',bd=3,padx=5,
                   command=reset)
